...
function fnBindTableValueForSingle(tableId, info, callback) {	

	var templeteTbody = window['tempTbody_' + tableId];
	
	if(typeof templeteTbody == 'undefined'){
		window['tempTbody_' + tableId] = $("#"+tableId).find(".fwk_databind_single_tbody").html();
		templeteTbody = window['tempTbody_' + tableId];
	} else {
		templeteTbody = window['tempTbody_' + tableId];
	}
...
// #{변수}를 제거 하고 전역변수에 tbody templete 영역을 담아둔다. 
function fnClearSigleBindKey(tableId) {
	...
	var html = templeteTbody;
	
	
	var rstHtml = '';
	var skipFlag = false;
	var skipFnFlag = false;
	var closeCnt = 0;
	for(var i = 0 ; i < html.length ; i++) {
		
		if(html.charAt(i) == "#" && html.charAt(i + 1) == "f" && html.charAt(i + 2) == "n" && html.charAt(i + 3) == "{") {
			skipFnFlag = true;
			closeCnt++;
		}
		...
		if(skipFnFlag) {
			...
			if(skipFlag) {
				...
				rstHtml = rstHtml + html.charAt(i);
			}
		}
	}
		
	$("#"+tableId).find(".fwk_databind_single_tbody").html(rstHtml);
