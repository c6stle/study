...
// jstreeGrid 생성
function fnBindJstreeGrid(divId, listObj, authDtlObj, codeObj, userId, treeCallback, treeGridCallback){
	// core 생성 
...
//		console.log("--> jstree loaded");
		try {
			if(treeCallback && treeCallback != undefined) {
				//alert("xx" + treeCallback);
				//eval(treeCallback(listObj));
				//treeCallback();
				treeCallback();
			}
		} catch(e) {alert(e)}
	...
	$("#"+divId).bind("loaded_grid.jstree",function(){
		...
		try {
			if(treeGridCallback && treeGridCallback != undefined) 
				treeGridCallback();
		} catch(e) {}
...
function fnSaveAuthor(divId){
	...
	$.each($(jstreeTable).find("input"), function(index, value) {
		var menuId = $(value).attr("data-menu-sn");
		var authObj = {};
		var userAuthObj = {};
		...
		if(selectUserId == ""){
			...
			if(!$(value).is(":disabled") && $(value).is(":checked")){
				...
				userAuthObj.menuAuthrtCd = $(value).attr("data-code").toUpperCase();
