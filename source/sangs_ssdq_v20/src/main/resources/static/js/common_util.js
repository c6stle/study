 ...
 * 현재 페이지
 */
function fnCurrentPageNum(pagingId) {
	if (!pagingId) {
		pagingId = "dv_paging";
	}
	return $("#"+pagingId).find("li.on").attr("data-pagenum");
}

/*
 * 
 */
function fnTableClickIndex(tableId) {
	return $("#"+tableId+" .fwk_data_row").index($("#"+tableId+" .fwk_data_row[data-selectedtr='Y']"));
}

/*
 * 
 */
function fnTableRowClick(tableId, tableClickIndex) {
	$("#"+tableId+" .fwk_data_row").eq(tableClickIndex).trigger("click");
}