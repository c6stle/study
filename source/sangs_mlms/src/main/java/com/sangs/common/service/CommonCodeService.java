...
public class CommonCodeService extends ServiceBase {
	...
	public Map<String, Object> getCommonCodeMultiList(Map<String, Object> paramMap) throws SangsMessageException {
		...
		try {
			...
			for(String str : sqlCodeList) {
				...
				if("LEARNING_GROUP_CODE".equals(sqlId)) {	// 학습 그룹 코드 목록
					list = getLearningGroupCodeList();
				} else {
					list = dao.selectList(sqlId, searchMap);
				}
	 ...
	 * @throws SangsMessageException throws SangsMessageException
	 */
	private List<SangsMap> getLearningGroupCodeList() {
		try {
			Map<String, Object> paramMap = new HashMap<String, Object>();
			
			return dao.selectList("mlms_code.selectLearningGroupList", paramMap);
		} catch(SangsMessageException e) {
			logger.error("", e);
			return null;
		} catch(Exception e) {
