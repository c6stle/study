 ...
 */
@Component
public class CommonSvcMdServiceInvoker {
	...
	public Object dataMethod(Map paramMap, String serviceId, String methodId) throws Exception {
		...
		try {
			...
			if(defParams.length != 0) {
				...
				try {
					...
					if(strSvcClassType.indexOf("java.util.List") >= 0 ) {
						...
						if(strSvcClassType.indexOf("java.util.Map") >= 0 || strSvcClassType.indexOf("class com.sangs.common.support.domain.SangsMap") >= 0 ) {
							arrArgs[0] = paramMap;
							//findFlag = true;
						}
			...
			bean.setTargetMethod("initService");
			bean.setArguments(defualtSqlSessionTemplate);
			
			bean.prepare();
			bean.invoke();
			bean.setArguments(null);	// 파라미터 초기 화 
